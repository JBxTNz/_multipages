    ...
    const repeatLastOperation = () => {
        ...
        switch (lastOperation.operation) {
            ...
            case 'x':
                ...
                break;
            case '÷':
                setCurrentOperand(current === 0 ? "Error" : current / lastOperation.number);
                break;
            default:
                break;
        }
        ...
        <div className="calculator">
            ...
            <div className='btn_container'>
                <div className="cal_btn_row">
                    <button className="cal_btn double opt" onClick={clear}>AC</button>
                    <button className="cal_btn opt" onClick={deleteLast}><i className='bi bi-backspace'></i></button>
                    <button className="cal_btn opt" onClick={() => chooseOperation('÷')}>÷</button>
                </div>
                <div className="cal_btn_row">
                    {[7, 8, 9].map(num => (
                        <button key={num} className="cal_btn" onClick={() => appendNumber(num.toString())}>{num}</button>
                    ))}
                    <button className="cal_btn opt" onClick={() => chooseOperation('x')}>x</button>
                </div>
                <div className="cal_btn_row">
                    {[4, 5, 6].map(num => (
                        <button key={num} className="cal_btn" onClick={() => appendNumber(num.toString())}>{num}</button>
                ...
                <div className="cal_btn_row">
                    <button className="cal_btn" onClick={() => appendNumber('0')}>0</button>
                    <button className="cal_btn" onClick={() => appendNumber('.')}>.</button>
                    <button className="cal_btn double Equal-btn" onClick={() => {
